...
def zk_listener():
    ...
    while True:
        ...
        try:
            ...
            # --- STEP 1: Fetch logs from last 24 hours on startup ---
            ...
            if existing_attendance:
                # Filter strictly to logs within last 24 hours
                recent_logs = [
                    att for att in existing_attendance
                    if threshold_time < make_aware_if_naive(att.timestamp) <= now
                ]

                print(f"📅 Found {len(recent_logs)} logs in the last 24 hours. Processing...")

                if recent_logs:
                    for att in sorted(recent_logs, key=lambda x: make_aware_if_naive(x.timestamp)):
                        uid = att.user_id
            ...
            while True:
                try:
                    ...
                    for att in attendance:
                        ...
                        # Skip if log is older than 24 hours or already processed
